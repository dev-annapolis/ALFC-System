...
<div class="container-fluid p-5">
    ...
    <div class="row TOP-ROW mb-3">
        <div class="col-md-6 SOURCES-MASTERLIST">
            <h1 class="fs-2 fw-medium">Master List of <strong>Sources</strong></h1>
        </div>
        <div class="col-md-6 SEARCHING d-flex align-items-center justify-content-end">
            <!-- Add Source Button -->
            <button class="btn btn-sm btn-primary fs-6 pe-3 me-3" data-bs-toggle="modal" data-bs-target="#addSourceModal">
    ...
    <div class="table-container">
        ...
        <div class="table-responsive">
            <table class="table dt-responsive shadow-sm " id="sourcesTable">
                ...
                <tbody>
                    @foreach ($sources as $source)
                        <tr class="fs-6 shadow-sm">
                            <td class="text-secondary fw-medium">{{ $source->name }}</td>
                            <td>
                                <span class="badge {{ $source->status === 'active' ? 'bg-active' : 'bg-inactive' }}">
                                    {{ ucfirst($source->status) }}
                                </span>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('sources.changeStatus', $source->id) }}" class="d-inline" >
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn btn-change-status btn-sm">Change Status</button>
                                </form>

                                <button class="btn btn-edit btn-sm" data-bs-toggle="modal" data-bs-target="#editSourceModal" data-id="{{ $source->id }}" data-name="{{ $source->name }}" style="margin-left: 0.5rem;">
                                    Edit
                                </button>
                            </td>
...
<div class="modal fade" id="editSourceModal" tabindex="-1" aria-labelledby="editSourceModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('sources.update') }}">
            @csrf
            @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchBox.find('input').addClass('form-control').attr('placeholder', 'Search Sources').css('width', '100%');
